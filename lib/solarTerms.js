...
// 절기 이름("하지", "대서" 등)으로 순번(0~23)을 찾음
// 데이터 파일에서 숫자 대신 이름으로 직관적으로 설정할 수 있게 하기 위함
export function termIndex(name) {
  const idx = SOLAR_TERMS.findIndex((t) => t.name === name);
  if (idx === -1) {
    throw new Error(`알 수 없는 절기 이름: "${name}" — 24절기 이름을 정확히 입력했는지 확인하세요.`);
  }
  return idx;
}
